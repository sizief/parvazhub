...
class SupplierSearch
  ...
  def supplier_list
    ...
    if search_initiator == 'job_search'
      suppliers = suppliers.where(job_search_allowed: true)
    end
    suppliers
  end

  def merge_and_update_all(flight_ids, _route, _date)
  ...
  def search_supplier(supplier_name, supplier_class)
    ...
    ActiveRecord::Base.connection_pool.with_connection do
      search_history = SearchHistory.create(supplier_name: supplier_name.downcase.to_s,
                                            route_id: route.id,
                                            departure_time: date,
                                            successful: false,
