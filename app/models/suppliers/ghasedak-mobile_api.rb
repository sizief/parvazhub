=begin
class Suppliers::Ghasedak_mobile_api
    require "uri"
    require "rest-client"
  ...
  def search(origin,destination,date,search_history_id)
      ...
      begin
        #api_url = "https://ghasedak24.com/oob/thirdparty/search"
        #date = date.to_date.to_parsi.to_s.gsub("-","/")  
        #api_params = {'from' => "#{origin.upcase}", 'to' => "#{destination.upcase}", 'date' => "#{date}", 'number'=>'1','Oauth-Token'=>"89541f96-79d7-11e7-8fc9-000c29ff45b7"}
        #api_response = RestClient::Request.execute(method: :post, url: "#{URI.parse(url)}",headers: {:'Oauth-Token'=> "955db966-79da-11e7-9ac5-000c29ff45b7"}, proxy: nil,payload: params)
