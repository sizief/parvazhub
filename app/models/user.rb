class User < ApplicationRecord
  ...
  def last_searches 
    search_histories = self.user_search_histories.order("created_at DESC").limit(10)
    routes = Hash.new
  ...
  def last_searches_without_past_dates 
    searches = self.last_searches
    searches.each do |search|
      fileterd_dates = Array.new
      search[:dates].each do |date|
        fileterd_dates << date if date.to_date > Date.today-1
      end
      fileterd_dates << (Date.today+1).to_s if fileterd_dates.empty? #add one date at least
