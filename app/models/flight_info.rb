...
class FlightInfo < ApplicationRecord
  ...
  def prepare_flight_info(flight)
    ...
    unless flight_details.empty?
      unless flight_details.last.airplane_type.nil?
        airplane = flight_details.last.airplane_type
        end
    ...
    { call_sign: call_sign, airplane: airplane, delay: delay, canceled: canceled, weekly_delay: weekly_delay }
  end
  ...
  def calculate_delay_based_on_flight_number(flight_details)
    ...
    if delay.empty?
      ...
      (delay.sum.to_f / delay.size).round
    end
  end
  ...
  def calculate_delay_based_on_week(flight_details, date)
    delay = []
    flight_details.each do |flight_detail|
      next if flight_detail.actual_departure_time.nil?
  ...
  def canceled_count(call_sign, departure_time); end
end
