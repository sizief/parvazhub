...
class Airports::DomesticAirport
  ...
  def search(city_name)
    ...
    begin
      if Rails.env.test?
        ...
        response = RestClient::Request.execute(url: URI.parse(get_flight_url).to_s, method: :get, verify_ssl: false)
  ...
  def get_date_time(day, time)
    ...
    if get_english_name(day) == yesterday_name
      yesterday_datetime.to_datetime
    elsif get_english_name(day) == today_name
      today_datetime.to_datetime
    else
      tomorrow_datetime.to_datetime
    end
  end
